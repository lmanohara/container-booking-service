...
class BookingsServiceImplTest {
  ...
  @InjectMocks BookingsServiceImpl bookingsService;
  @Mock private ExternalServiceClient externalServiceClient;
  ...
  void givenBookingSpecification_whenSpacesAvailable_thenShouldReturnTrue() {
    ...
    when(externalServiceClient.callExternalAPI("/checkAvailable"))
        .thenReturn(Mono.just(externalAPIResponse));
    Mono<AvailableSpacesResponse> availableSpacesMono =
        bookingsService.checkAvailableSpaces(bookingsSpec);

    StepVerifier.create(availableSpacesMono)
        .assertNext(actual -> assertThat(actual.getAvailability()).isTrue())
        .verifyComplete();
  }
  ...
  void givenBookingSpecification_whenSpacesAreNotAvailable_thenShouldReturnFalse() {
    ...
    StepVerifier.create(availableSpacesMono)
        .assertNext(actual -> assertThat(actual.getAvailability()).isFalse())
        .verifyComplete();
  }
  ...
  void givenBookingSpec_whenSaveNewBooking_thenShouldReturnRef() throws Exception {
    ...
    StepVerifier.create(bookingRef)
        .assertNext(
            actual ->
                assertThat(Integer.parseInt(actual.getBookingRef())).isEqualTo(bookingRefIncrement))
        .verifyComplete();
  }
}
