...
public class BookingsServiceImpl implements BookingsService {
  ...
  public BookingsServiceImpl(
      ExternalServiceClient externalServiceClient, BookingsRepository bookingsRepository) {
  ...
  public Mono<AvailableSpacesResponse> checkAvailableSpaces(@NonNull BookingsSpec bookingsSpec) {
    Mono<ExternalAPIResponse> externalAPIResponseMono =
        externalServiceClient.callExternalAPI("/checkAvailable");
    return externalAPIResponseMono.map(this::mapToAvailableSpanceResponse);
  }

  private AvailableSpacesResponse mapToAvailableSpanceResponse(
      ExternalAPIResponse externalAPIResponse) {
    boolean isSpaceAvailable = externalAPIResponse.getAvailableSpace() > 0;

    return AvailableSpacesResponse.builder().availability(isSpaceAvailable).build();
  }
  ...
  public Mono<BookingsRef> createBookings(@NonNull BookingsSpec bookingsSpec) {
    ...
    return bookingsSpecMono.map(
        bookingsSpecValue -> new BookingsRef(bookingsSpecValue.getBookingRef()));
  }
}
